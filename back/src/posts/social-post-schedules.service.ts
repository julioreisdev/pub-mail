...
} from './dto/social-schedule-shared.dto';
import {
  getSocialPostScheduleTokensCost,
  SOCIAL_POST_SCHEDULES_TIMEZONE,
} from './social-schedule-config';

function parseIsoDateOrThrow(value: string, fieldName: string): Date {
  const parsed = new Date(value);
  if (Number.isNaN(parsed.getTime())) {
    throw new BadRequestException(`${fieldName} deve ser um ISO datetime válido.`);
  }
  return parsed;
}
...
function getUtcDayRange(baseDate: Date) {
...
@Injectable()
export class SocialPostSchedulesService {
  private readonly logger = new Logger(SocialPostSchedulesService.name);

  constructor(
    private readonly prisma: PrismaService,
    private readonly postsService: PostsService,
    private readonly socialAccountsCrypto: SocialAccountsCryptoService,
  ...
  getMeta() {
    return {
      timezone: SOCIAL_POST_SCHEDULES_TIMEZONE,
      tokens_cost_per_dispatch: getSocialPostScheduleTokensCost(),
      allowed_networks: [...SOCIAL_NETWORK_VALUES],
      allowed_schedule_status: [...SOCIAL_SCHEDULE_STATUS_VALUES],
      allowed_run_status: [...SOCIAL_SCHEDULE_RUN_STATUS_VALUES],
      full_hour_only: true,
  ...
  private normalizeNetworkOrThrow(networkRaw?: string | null) {
    const normalized = String(networkRaw || '')
      .trim()
      .toUpperCase();
    if (!normalized || !SOCIAL_NETWORK_VALUES.includes(normalized as any)) {
      throw new BadRequestException('Rede social inválida.');
    }
    return normalized;
  }

  private buildTikTokDailyLimitError(nextResetAt: Date) {
    return `Limite de posts diário atingido. Tente novamente mais tarde. Próximo reset: ${nextResetAt.toISOString()} (UTC).`;
  }

  private async getTikTokDailyUsageForAccount(input: {
    ...
    const [scheduledCount, consumedRunsCount] = await Promise.all([
      ...
      this.prisma.social_post_schedule_runs.count({
        where: {
          ...
          social_network: 'TIKTOK',
          social_account_id: input.socialAccountId,
          status: { in: ['PROCESSING', 'COMPLETED'] as any[] },
  ...
  private async assertTikTokDailyLimitOnSchedule(input: {
    organizationId: string;
    socialAccountId: string;
    scheduledAt: Date;
    excludeScheduleId?: string;
    ...
    if (usage.used >= usage.limit) {
      throw new BadRequestException(
        this.buildTikTokDailyLimitError(usage.nextResetAtUtc),
      );
    }
  ...
  async getDailyAccountLimits(organizationId: string, socialNetworkRaw?: string) {
  ...
  private async prepareCreateSchedulePayload(
    organizationId: string,
    dto: CreateSocialPostScheduleDto,
    ...
    if (scheduledAt.getTime() < Date.now()) {
      throw new BadRequestException(
        'Erro: Horário inválido. Escolha um horário válido.',
      );
    }
    ...
    if (!socialAccount?.id) {
      throw new BadRequestException(
        'Conta social ativa não encontrada para a rede selecionada.',
      );
    }
    ...
    if (!post) {
      throw new NotFoundException('Post da Galeria não encontrado.');
    }

    const normalizedPayload = this.normalizePlatformPayload(
      dto.social_network,
      dto.platform_payload ?? {},
    );
    const tokensCost = getSocialPostScheduleTokensCost();
    ...
    if (dto.social_network === 'TIKTOK') {
      ...
      this.assertTikTokSchedulePayloadRules(
        normalizedPayload,
        creatorInfo,
        postSnapshot,
      );
  ...
  async create(organizationId: string, dto: CreateSocialPostScheduleDto) {
  ...
  async dispatchNow(organizationId: string, scheduleId: string) {
    ...
    if (
      String(schedule.social_network) === 'TIKTOK' &&
      ...
      const scheduleAt = schedule?.scheduled_at
        ...
        : null;
      const shouldExcludeReservedSchedule =
    ...
    try {
      ...
      this.logger.error(
        `[dispatch-now] erro inesperado ao disparar agendamento ${scheduleId} (org=${organizationId}): ${String(
          error?.message || error,
        )}`,
        error?.stack,
      );
  ...
  async list(organizationId: string, query: ListSocialPostSchedulesDto) {
    return {
      items: await this.prisma.social_post_schedules.findMany({
        where: {
          organization_id: organizationId,
          ...(query.social_network
            ? { social_network: query.social_network as any }
            : {}),
  ...
  async listRuns(
    organizationId: string,
    query: ListSocialPostScheduleRunsDto,
  ) {
    return {
  ...
  async processDueSchedules(runAtHour: Date) {
    this.logger.debug(
      `[tick-social] processing due schedules at ${runAtHour.toISOString()}`,
    );
  ...
  async processTikTokPendingRunsStatus() {
    ...
    this.logger.debug(
      `[poll-tiktok] checking ${pendingRuns.length} pending run(s)`,
    );
  ...
  private async processSingleTikTokPendingRun(run: {
    id: string;
    ...
    if (!socialAccountId || !publishId) return;
    ...
    if (!socialAccount?.id || socialAccount.status !== 'ACTIVE') {
      await this.markRunAsFailed(
        run.id,
        'Conta social indisponível para acompanhar status da publicação no TikTok.',
      );
    ...
    try {
      ...
      if (polled.finalStatus === 'FAILED') {
        this.logger.error(
          `[poll-tiktok] publish failed run=${run.id} publish_id=${publishId} | reason=${String(
            polled.message || 'unknown',
          )}`,
        );
      ...
      if (
        ...
        await this.markRunAsFailed(
          run.id,
          `Falha no polling do TikTok: ${message}`.slice(0, 255),
        );
        await this.cleanupPostIfNoMoreSchedules(run.organization_id, run.post_id);
        return;
      }
      ...
      this.logger.warn(
        `[poll-tiktok] run=${run.id} publish_id=${publishId} erro temporário: ${message}`,
      );
    }
  }

  private async processSingleSchedule(schedule: any, runAtHour: Date) {
    ...
    if (!schedule.social_account_id) {
      await this.markRunAsFailed(
        run.id,
        'Agendamento sem conta social vinculada.',
      );
    ...
    const socialAccount = await this.prisma.social_accounts.findFirst({
      where: {
        ...
        organization_id: schedule.organization_id,
        social_network: schedule.social_network as any,
      },
      select: {
        id: true,
        status: true,
    ...
    if (!socialAccount?.id || socialAccount.status !== 'ACTIVE') {
      await this.markRunAsFailed(
        run.id,
        'Conta social indisponível para o disparo (desconectada ou removida).',
      );
      await this.markScheduleAsFailed(
        schedule.id,
        'Conta social indisponível para o disparo (desconectada ou removida).',
    ...
    if (
      String(schedule.social_network).toUpperCase() === 'TIKTOK' &&
      !normalizeOptionalString(socialAccount.access_token_encrypted)
    ) {
      await this.markRunAsFailed(
        run.id,
        'Conta TikTok desconectada ou sem sessão válida. Reconecte a conta para voltar a disparar.',
      );
      await this.markScheduleAsFailed(
        schedule.id,
        'Conta TikTok desconectada ou sem sessão válida. Reconecte a conta para voltar a disparar.',
    ...
    if (!wallet || Number(wallet.balance) < tokensCost) {
      await this.markRunAsFailed(
        run.id,
        'Saldo insuficiente para executar o disparo agendado.',
      );
      await this.markScheduleAsFailed(
        schedule.id,
        'Saldo insuficiente para executar o disparo agendado.',
    ...
    try {
      ...
      if (String(schedule.social_network).toUpperCase() !== 'TIKTOK') {
        await this.cleanupPostIfNoMoreSchedules(
      ...
      const message = this.normalizeDispatchFailureMessage(
        String(schedule.social_network),
        error,
        fallbackMessage,
      ).slice(0, 255);
      const debugContext = {
      ...
      this.logger.error(
        `[dispatch-social] falha no disparo: ${JSON.stringify(debugContext)} | message=${message}`,
        error?.stack,
      );
  ...
  private async cleanupPostIfNoMoreSchedules(
    organizationId: string,
    postId: string,
  ) {
    const [pendingSchedulesCount, totalRunsCount, nonCompletedRunsCount] =
      await Promise.all([
      this.prisma.social_post_schedules.count({
    ...
    if (pendingSchedulesCount > 0) return;
    if (totalRunsCount === 0) return;
    if (nonCompletedRunsCount > 0) return;
    ...
    try {
      ...
      this.logger.warn(
        `[cleanup-post] não foi possível apagar post ${postId}: ${msg}`,
      );
    }
  }

  private async markRunAsFailed(runId: string, message: string) {
    await this.prisma.social_post_schedule_runs.update({
      where: { id: runId },
      data: {
        status: 'FAILED',
  ...
  private async markScheduleAsFailed(scheduleId: string, message: string) {
    await this.prisma.social_post_schedules.update({
      where: { id: scheduleId },
      data: {
        status: 'FAILED',
  ...
  private async dispatchToSocialNetwork(input: {
    ...
    if (input.socialNetwork !== 'TIKTOK') {
      throw new Error(
        `Disparo real da rede ${input.socialNetwork} ainda não está implementado.`,
      );
    }
  ...
  private async getTikTokDispatchConfig(organizationId: string) {
    const credentials =
      await this.socialAccountsService.resolveTikTokClientCredentialsForOrganization(
        organizationId,
      );
    ...
    return {
      ...
      creatorInfoEndpoint:
        normalizeOptionalString(process.env.TIKTOK_CREATOR_INFO_ENDPOINT) ||
        'https://open.tiktokapis.com/v2/post/publish/creator_info/query/',
      directPostEndpoint:
        normalizeOptionalString(process.env.TIKTOK_DIRECT_POST_ENDPOINT) ||
        'https://open.tiktokapis.com/v2/post/publish/video/init/',
      photoPostEndpoint:
        normalizeOptionalString(process.env.TIKTOK_PHOTO_POST_ENDPOINT) ||
        'https://open.tiktokapis.com/v2/post/publish/content/init/',
      statusFetchEndpoint:
        normalizeOptionalString(process.env.TIKTOK_STATUS_FETCH_ENDPOINT) ||
        'https://open.tiktokapis.com/v2/post/publish/status/fetch/',
      mediaBaseUrl:
        normalizeOptionalString(process.env.SOCIAL_POST_MEDIA_BASE_URL) ||
  ...
  private normalizeTikTokScope(rawScope: any) {
  ...
  private parseTikTokApiErrorMessage(raw: any, fallback: string) {
    return String(
      raw?.error?.message ||
  ...
  private parseTikTokApiCode(raw: any) {
    return normalizeOptionalString(
      raw?.error?.code || raw?.code || raw?.error_code,
    );
  }

  private buildTikTokApiError(raw: any, status: number, fallback: string) {
    ...
    const error: any = new Error(
      `TikTok API (${status})${code ? ` [${code}]` : ''}: ${message}${logId ? ` (log_id=${logId})` : ''}`,
    );
  ...
  private isTikTokAuthDisconnectedError(error: any) {
    ...
    return (
      message.includes('access token tiktok inválido') ||
  ...
  private normalizeTikTokCreatorInfo(creatorInfoRaw: any) {
    ...
    const maxVideoDurationSecRaw = Number(
      creatorInfoRaw?.max_video_post_duration_sec,
    );
    ...
    return {
      ...
      max_video_post_duration_sec:
        Number.isFinite(maxVideoDurationSecRaw) && maxVideoDurationSecRaw > 0
          ? Math.floor(maxVideoDurationSecRaw)
          : null,
    };
  }

  private async fetchTikTokCreatorInfoForAccount(input: {
    organizationId: string;
    socialAccountId: string;
    socialAccount: any;
  }) {
    ...
    if (!encryptedAccessToken) {
      throw new BadRequestException(
        'Conta TikTok sem access token. Reconecte a conta antes de agendar.',
      );
    }

    let accessToken = normalizeOptionalString(
      this.socialAccountsCrypto.decrypt(encryptedAccessToken),
    );
    if (!accessToken) {
      throw new BadRequestException(
        'Access token TikTok inválido. Reconecte a conta antes de agendar.',
      );
    }

    const refreshTokenEncrypted = normalizeOptionalString(
      input.socialAccount?.refresh_token_encrypted,
    );

    const refreshAccessToken = async () => {
      if (!refreshTokenEncrypted) {
        throw new BadRequestException(
          'Conta TikTok sem refresh token para renovação automática. Reconecte a conta.',
        );
      }

      if (!config.clientKey || !config.clientSecret) {
        throw new BadRequestException(
          'Configure TIKTOK_CLIENT_KEY e TIKTOK_CLIENT_SECRET (ou salve Keys e APIs da organização) para renovar token do TikTok.',
        );
      }
    ...
    const shouldRefreshBeforeFetch =
      Boolean(refreshTokenEncrypted) &&
    ...
    try {
      const response = await this.postTikTokApi(
        config.creatorInfoEndpoint,
        String(accessToken),
      );
      ...
      if (errorCode === 'access_token_invalid' && refreshTokenEncrypted) {
        await refreshAccessToken();
        const retryResponse = await this.postTikTokApi(
          config.creatorInfoEndpoint,
          String(accessToken),
        );
  ...
  async getTikTokCreatorInfoForSchedule(
    organizationId: string,
    socialAccountId: string,
  ) {
    ...
    if (!account?.id) {
      throw new NotFoundException('Conta TikTok ativa não encontrada.');
    }
  ...
  private normalizeTikTokPublishStatus(statusRaw: any, failReasonRaw: any) {
  ...
  private async fetchTikTokPublishStatusForRun(input: {
    organizationId: string;
    socialAccountId: string;
    publishId: string;
    socialAccount: any;
    ...
    if (!encryptedAccessToken) {
      throw new Error(
        'Conta TikTok sem access token para consultar status da publicação.',
      );
    }
    ...
    if (!accessToken) {
      throw new Error(
        'Access token TikTok inválido para consultar status da publicação.',
      );
    }
    ...
    const refreshAccessToken = async () => {
      if (!refreshTokenEncrypted) {
        throw new Error(
          'Conta TikTok sem refresh token para renovação automática.',
        );
      }

      if (!config.clientKey || !config.clientSecret) {
        throw new Error(
          'Configure credenciais do TikTok para renovação automática do access token.',
        );
      }
    ...
    try {
      const response = await this.postTikTokApi(
        config.statusFetchEndpoint,
        String(accessToken),
        requestBody,
      );
      return this.normalizeTikTokPublishStatus(
      ...
      if (errorCode === 'access_token_invalid' && refreshTokenEncrypted) {
        ...
        return this.normalizeTikTokPublishStatus(
          ...
          retryResponse?.data?.fail_reason,
        );
      }
  ...
  private resolveTikTokVideoDurationSec(postSnapshot: any) {
    const media = Array.isArray(postSnapshot?.media)
      ? [...postSnapshot.media].sort(
  ...
  private assertTikTokSchedulePayloadRules(
    ...
    if (!privacyLevel) {
      throw new BadRequestException(
        'Selecione manualmente a privacidade do post no TikTok.',
      );
    }
    ...
    if (
      allowedPrivacyOptions.length > 0 &&
    ...
    const commercialDisclosure = this.normalizeTikTokCommercialDisclosure(
      payload,
      {
    ...
    if (
      commercialDisclosure.brandContentToggle &&
      requestedPrivacy === 'SELF_ONLY'
    ) {
      throw new BadRequestException(
        'Conteúdo de marca não pode ser publicado com privacidade "Somente eu".',
      );
    }
    ...
    if (mediaInput.kind === 'VIDEO') {
      if (!videoDurationSec) {
        throw new BadRequestException(
          'Vídeo sem duração disponível na galeria. Reenvie o vídeo para atualizar os metadados.',
        );
      }
      ...
      if (!Number.isFinite(maxVideoDurationSec) || maxVideoDurationSec <= 0) {
        throw new BadRequestException(
          'Não foi possível validar a duração máxima permitida pelo TikTok para esta conta.',
        );
      }

      if (videoDurationSec > maxVideoDurationSec) {
        throw new BadRequestException(
          `Vídeo com ${videoDurationSec}s excede o limite permitido pelo TikTok para esta conta (${Math.floor(
            maxVideoDurationSec,
          )}s).`,
        );
      }
  ...
  private normalizeTikTokCommercialDisclosure(
    payload: Record<string, any>,
    options?: { throwWhenEnabledWithoutSelection?: boolean },
  ) {
    ...
    const explicitEnabled =
      typeof payload?.commercial_content_enabled === 'boolean'
    ...
    if (!requestedBrandOrganic && !requestedBrandContent) {
      if (options?.throwWhenEnabledWithoutSelection) {
        throw new BadRequestException(
          'Você precisa indicar se o conteúdo promove sua marca, terceiros ou ambos.',
        );
      }
    }
  ...
  private mapTikTokPrivacyLevel(value?: string | null) {
    const raw = String(value || '')
      .trim()
      .toUpperCase();

    if (!raw) return 'PUBLIC_TO_EVERYONE';
  ...
  private resolveTikTokPrivacyLevel(
    ...
    if (creatorOptions.length === 0) return requested || 'SELF_ONLY';
    if (creatorOptions.includes(requested)) return requested;
    if (creatorOptions.includes('SELF_ONLY')) return 'SELF_ONLY';
  ...
  private buildTikTokCaption(
    ...
    const merged =
      [title, caption, fallbackCaption, fallbackTitle]
        .filter(Boolean)
        .join('\n\n')
  ...
  private buildTikTokMediaSummary(postSnapshot: any) {
    return Array.isArray(postSnapshot?.media) && postSnapshot.media.length > 0
      ? postSnapshot.media
          .map(
            (item: any) =>
              `${String(item?.media_type || '').toUpperCase() || 'UNKNOWN'}:${String(
                item?.storage_key ? 'ok' : 'missing',
              )}`,
          )
          .join(', ')
      : 'sem mídias';
  ...
  private buildPublicMediaUrl(storageKey: string, mediaBaseUrl: string) {
  ...
  private async probePublicMediaUrl(url: string) {
    const probe = {
      url,
      method: 'HEAD',
    ...
    try {
      ...
      probe.contentLength = normalizeOptionalString(
        head.headers.get('content-length'),
      );
      probe.finalUrl = normalizeOptionalString(head.url) || url;

      if (head.status !== 405) return probe;
    } catch (error: any) {
  ...
  private async logTikTokMediaDebugOnPublishFailure(input: {
    organizationId: string;
    runId: string;
    publishId: string;
    ...
    if (
      !reason.includes('video_pull_failed') &&
    ...
    if (!config.mediaBaseUrl) {
      this.logger.error(
        `[poll-tiktok] run=${input.runId} publish_id=${input.publishId} sem mediaBaseUrl para depurar mídia`,
      );
      return;
    }
    ...
    try {
      mediaInput = this.buildTikTokDispatchMediaInput(
        input.postSnapshot,
        config.mediaBaseUrl,
      );
    } catch (error: any) {
      this.logger.error(
        `[poll-tiktok] run=${input.runId} publish_id=${input.publishId} falha ao reconstruir mídia para debug: ${String(
          error?.message || error,
        )}`,
      );
      return;
    ...
    if (!targetUrl) {
      this.logger.error(
        `[poll-tiktok] run=${input.runId} publish_id=${input.publishId} sem URL de mídia para debug`,
      );
      return;
    }
    ...
    this.logger.error(
      `[poll-tiktok] media debug run=${input.runId} publish_id=${input.publishId} reason=${reason} probe=${JSON.stringify(
        probe,
      )}`,
    );
  }

  private buildTikTokDispatchMediaInput(postSnapshot: any, mediaBaseUrl: string) {
    ...
    if (videos.length > 0 && images.length > 0) {
      throw new Error(
        `Post da galeria com mídias mistas (vídeo + imagem), formato não suportado para disparo TikTok. Mídias encontradas: ${mediaSummary}`,
      );
    }
    ...
    if (videos.length > 0) {
      return {
        kind: 'VIDEO' as const,
        videoUrl: this.buildPublicMediaUrl(
          String(videos[0].storageKey),
          mediaBaseUrl,
        ),
      };
    }
    ...
    if (images.length > 0) {
      if (images.length > 35) {
        throw new Error(
          `Post da galeria com ${images.length} imagens. O TikTok permite no máximo 35 imagens por publicação.`,
        );
      }

      return {
        kind: 'PHOTO' as const,
        photoImages: images.map((item) =>
          this.buildPublicMediaUrl(String(item.storageKey), mediaBaseUrl),
        ),
        photoCoverIndex: 0,
      };
    }
    ...
    throw new Error(
      `Post da galeria sem mídia disponível para publicação no TikTok. Mídias encontradas: ${mediaSummary}`,
    );
  }

  private async refreshTikTokAccessToken(input: {
    socialAccountId: string;
    ...
    const refreshToken = normalizeOptionalString(
      this.socialAccountsCrypto.decrypt(input.refreshTokenEncrypted),
    );

    if (!refreshToken) {
      throw new Error(
        'Refresh token do TikTok indisponível para renovar o access token.',
      );
    }
    ...
    const codeOk = !code || code.toLowerCase() === 'ok';
    if (!response.ok || !codeOk) {
      throw this.buildTikTokApiError(
        raw,
        response.status,
        'Falha ao renovar access token no TikTok.',
      );
    }
  ...
  private async dispatchTikTokDirectPost(input: {
    socialNetwork: string;
    runId: string;
    organizationId: string;
    socialAccountId: string;
    ...
    if (!config.mediaBaseUrl) {
      throw new Error(
        'Configure SOCIAL_POST_MEDIA_BASE_URL (ou BASE_API_URL) para publicar no TikTok por URL.',
      );
    }
    ...
    const shouldRefreshBeforeDispatch =
      Boolean(refreshTokenEncrypted) &&
      input.socialAccount?.token_expires_at &&
      new Date(input.socialAccount.token_expires_at).getTime() <=
        Date.now() + 60_000;
    ...
    const mediaInput = this.buildTikTokDispatchMediaInput(
      input.postSnapshot,
      config.mediaBaseUrl,
    );

    const tryPublish = async (token: string, forcePrivate: boolean) => {
      const creatorInfoResponse = await this.postTikTokApi(
        config.creatorInfoEndpoint,
        token,
      );
      ...
      const privacyLevel = this.resolveTikTokPrivacyLevel(
        requestedPrivacy,
        creatorInfo?.privacy_level_options,
      );
      const commercialDisclosure = this.normalizeTikTokCommercialDisclosure(
      ...
      const disableComment =
        Boolean(creatorInfo?.comment_disabled) || !allowCommentRequested;
      const disableDuet =
        Boolean(creatorInfo?.duet_disabled) || !allowDuetRequested;
      const disableStitch =
        Boolean(creatorInfo?.stitch_disabled) || !allowStitchRequested;
      const caption = this.buildTikTokCaption(
        input.platformPayload,
        input.postSnapshot,
      );
      const titleForPhoto =
        normalizeOptionalString(input.platformPayload?.title) ||
        normalizeOptionalString(input.postSnapshot?.default_title) ||
        caption;

      if (mediaInput.kind === 'VIDEO') {
        const videoDurationSec = this.resolveTikTokVideoDurationSec(
          input.postSnapshot,
        );
        if (!videoDurationSec) {
          throw new Error(
        ...
        const maxVideoDurationSec = Number(
          creatorInfo?.max_video_post_duration_sec,
        );
        if (!Number.isFinite(maxVideoDurationSec) || maxVideoDurationSec <= 0) {
          throw new Error(
            'TikTok não retornou max_video_post_duration_sec para validar a duração do vídeo.',
          );
        }
      ...
      const directPostEndpoint =
        mediaInput.kind === 'VIDEO'
          ? config.directPostEndpoint
          : config.photoPostEndpoint;

      const directPostResponse = await this.postTikTokApi(
        directPostEndpoint,
        token,
        postBody,
      );

      const publishId = normalizeOptionalString(
        directPostResponse.data?.publish_id || directPostResponse.raw?.publish_id,
      );

      if (!publishId) {
        throw new Error(
          'TikTok não retornou publish_id após iniciar o direct post.',
        );
      }
    ...
    try {
      ...
      if (
        ...
        this.logger.warn(
          `[dispatch-tiktok] app não auditado, reprocessando run=${input.runId} com privacidade privada`,
        );
      ...
      if (errorCode === 'access_token_invalid' && refreshTokenEncrypted) {
        ...
        this.logger.warn(
          `[dispatch-tiktok] access token inválido, renovando token e tentando novamente (run=${input.runId})`,
        );
        ...
        const publishId = await tryPublish(refreshedAccessToken, false).catch(
          async (retryError: any) => {
            ...
            if (
              ...
              this.logger.warn(
                `[dispatch-tiktok] retry pós-refresh exigiu privacidade privada (run=${input.runId})`,
              );
  ...
  private buildDateRangeFilter(
    ...
    if (from) range.gte = parseIsoDateOrThrow(from, 'from');
    if (to) range.lte = parseIsoDateOrThrow(to, 'to');

    return { [field]: range };
  }

  private assertWholeHour(value: Date) {
    const valid =
      value.getMinutes() === 0 &&
      value.getSeconds() === 0 &&
    ...
    if (!valid) {
      throw new BadRequestException(
        'Somente horários cheios são permitidos (ex.: 17:00, 00:00).',
      );
    }
  }

  private buildPostSnapshot(post: any) {
