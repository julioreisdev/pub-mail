...
export default function QuizSplitRedirect() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await get(`/public/quiz-splits/${encodeURIComponent(slug)}`);
        const members = Array.isArray(data?.members) ? data.members : [];
        const chosen = weightedPick(members);
        if (!chosen) {
          if (alive) setError('Nenhum quiz ativo neste split.');
          return;
        }
        const domain = String(chosen.domain).toLowerCase();
        const target = `https://${domain}/quiz/${encodeURIComponent(chosen.slug)}?nsr=1`;
