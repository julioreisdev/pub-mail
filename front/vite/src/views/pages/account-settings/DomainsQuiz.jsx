...
function AdsTxtDialog({ open, domain, onClose, onSaved, onSnack }) {
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      await patch(`/quiz-domains/${domain.id}/ads-txt`, { ads_txt: content });
    ...
    >
      ...
      <Stack spacing={1}>
        ...
        >
          <Stack spacing={0.75}>
            ...
            <Typography variant="body2" color="text.secondary">
              5. Após verificar, o domínio fica liberado para seleção no wizard de quiz.
            </Typography>
          </Stack>
        </Box>
      ...
      {!isVerified ? (
        <Stack direction={{ xs: 'column', sm: 'column' }} spacing={1} justifyContent="flex-end" sx={{ mt: 1.25 }}>
          ...
          <Typography variant="caption" color="text.secondary">
            Após ficar verificado, o domínio aparece no wizard de quiz.
          </Typography>
        </Stack>
      ) : null}
    </Box>
...
export default function DomainsQuiz() {
  const { quizDomains, isLoading, error, refresh } = useQuizDomains();
  const list = useMemo(() => toList(quizDomains), [quizDomains]);
  ...
  useEffect(() => {
    ...
    setDomainError(isValidDomain(normalized) ? '' : 'Digite um domínio válido (ex: quizz.dominio.com)');
  }, [domainInput]);
  ...
  const handleCreate = async (e) => {
    ...
    if (!isValidDomain(normalized)) {
      setDomainError('Digite um domínio válido (ex: quizz.dominio.com)');
      return;
    }
    ...
    try {
      await post('/quiz-domains', { domain: normalized });
      setDomainInput('');
      await refresh();
    } catch (err) {
      setActionError(getErrorMessage(err, 'Falha ao cadastrar domínio de quiz.'));
    } finally {
  ...
  const doDelete = async () => {
    ...
    setActionError('');
    try {
      await remove(`/quiz-domains/${pendingDelete.id}`);
  ...
  const doVerify = async (item) => {
    ...
    setActionError('');
    try {
      await patch(`/quiz-domains/${item.id}/verify`, {});
      await refresh();
    } catch (err) {
      setActionError(getErrorMessage(err, 'Falha ao verificar domínio de quiz.'));
    } finally {
      setActionLoading(false);
    }
  };
  ...
  return (
    <>
      <Stack spacing={2}>
        ...
        {error ? (
          <Alert severity="error" sx={{ borderRadius: 2 }}>
            {getErrorMessage(error, 'Falha ao carregar domínios de quiz.')}
          </Alert>
        ) : null}
        ...
        {isLoading ? (
          ...
          >
            <Typography variant="h6" sx={{ mb: 0.5 }}>
              Nenhum domínio de quiz cadastrado
            </Typography>
            <Typography variant="body2" color="text.secondary">
              Cadastre e verifique um domínio para disponibilizar no wizard de quiz.
            </Typography>
          </Box>
        ) : (
          <Box
